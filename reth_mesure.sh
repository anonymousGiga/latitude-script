...
#     3. Collecting data with "features=enable_cache_record, default Th && unlimit Th".
#     4. Collecting data with "features=enable_tps_gas_record, default Th".
#     5. Collecting data with "features=enable_db_speed_record, default Th && unlimit Th".
#
#############################################################
...
#############################################################
#
#  3. Run with feature enable_cache_record 
#
#############################################################
# Build with feature enable_cache_record 
pushd reth && \
RUSTFLAGS="-C target-cpu=native" cargo build --profile maxperf --features=finish_after_execution_stage,enable_cache_record && \
CARGO_BIN="$HOME/.cargo/bin/" && \
cp ./target/maxperf/reth $CARGO_BIN && \
popd && \
echo "Install reth finish!" && \
...
#############################################################
#
#  4. Run with feature enable_tps_gas_record 
#
#############################################################
# Build with enable_tps_gas_record 
pushd reth && \
RUSTFLAGS="-C target-cpu=native" cargo build --profile maxperf --features=finish_after_execution_stage,enable_tps_gas_record && \
CARGO_BIN="$HOME/.cargo/bin/" && \
cp ./target/maxperf/reth $CARGO_BIN && \
popd && \
echo "Install reth finish!" && \
...
# Run reth with default Th.
RUST_LOG=info reth node --debug.max-block $TARGET_NUMBER --datadir $DATADIR --debug.terminate -d > default_tps_gas_record && \



#############################################################
#
#  5. Run with feature enable_db_speed_record 
#
#############################################################
# Build with feature enable_db_speed_record 
pushd reth && \
RUSTFLAGS="-C target-cpu=native" cargo build --profile maxperf --features=finish_after_execution_stage,enable_db_speed_record && \
CARGO_BIN="$HOME/.cargo/bin/" && \
cp ./target/maxperf/reth $CARGO_BIN && \
popd && \
echo "Install reth finish!" && \
...
# Run reth with default Th.
RUST_LOG=info reth node --debug.max-block $TARGET_NUMBER --datadir $DATADIR --debug.terminate -d > default_enable_db_speed_record && \
...
## Run reth with unlimit Th.
RUST_LOG=info reth node --debug.max-block $TARGET_NUMBER --datadir $DATADIR --config ./max.toml --debug.terminate -d > unlimit_enable_db_speed_record
