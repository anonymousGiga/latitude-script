...
#     1. Collecting data with "features=enable_cache_record, max Th".
#     2. Collecting data with "features=enable_opcode_metrics, max Th".
#     3. Collecting data with no metrics.
#
#############################################################
...
#############################################################
#
#  1. Run with features: enable_cache_record, max Th".
#
#############################################################
pushd reth && \
...
RUST_LOG=info reth node --debug.max-block $TARGET_NUMBER --datadir $DATADIR --config ./max.toml --debug.terminate -d > cachedb.log && \
...
# Run reth with max Th.
RUST_LOG=info reth node --debug.max-block $TARGET_NUMBER --datadir $DATADIR --config ./max.toml --debug.terminate -d > no_metrics.log
