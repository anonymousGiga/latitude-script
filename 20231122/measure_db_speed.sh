...
#     1. Collecting data with "features=enable_db_speed_record, max Th".
#
#############################################################
...
RUST_LOG=info reth node --debug.max-block $TARGET_NUMBER --datadir $DATADIR --config ./max.toml --debug.terminate -d > db_speed.log
